...
export function SocialButtons({ mode = "sign-in", onError }: SocialButtonsProps) {
  ...
  const handleOAuth = async (strategy: "oauth_google" | "oauth_github") => {
    try {
      ...
      const message =
        err instanceof Error ? err.message : "Failed to initiate OAuth sign-in. Please try again.";
      if (onError) {
        onError(message);
      } else {
        toast.error(message);
  ...
  return (
    <div className="flex flex-col gap-2.5 w-full">
      ...
      >
        ...
        <span>{mode === "sign-up" ? "Sign up with Google" : "Log in with Google"}</span>
      </button>
      ...
      >
        ...
        <span>{mode === "sign-up" ? "Sign up with GitHub" : "Log in with GitHub"}</span>
      </button>
    </div>
  );
}
