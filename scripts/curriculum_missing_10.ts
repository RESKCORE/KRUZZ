...
export const MISSING_10_CASES: any[] = [
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    engineeringConcepts: ["Inventory Reservation", "Session Timeout", "Soft Locking", "Checkout State Machine"],
    ...
    glossary: [
      { term: "Soft Hold", plainDefinition: "Temporarily reserving an item for a customer during checkout with an automatic expiry." },
      { term: "Hard Decrement", plainDefinition: "Permanently deducting item count after payment is authorized." },
    ],
    primers: [
      {
        concept: "Why You Can't Just Decrement on Add-To-Cart",
        minutes: 4,
        definition: "If you deduct stock when someone adds to cart, abandoned carts would cause items to show out of stock forever.",
        whyNeeded: "A temporary reservation with a 10-minute hold window balances shopper intent with business stock availability.",
        analogy: "A concert ticket seat is held for 8 minutes while you enter your credit card; if you walk away, it goes back on sale.",
        tinyExample: "hold_expiry = current_time + 600",
      },
    ],
    discover: {
      situation: "Shoppers add items to carts, but only a fraction actually purchase.",
      humanFlow: ["Browse item", "Add to cart", "Proceed to payment", "Authorize funds", "Fulfill order"],
      question: "How do we hold inventory during checkout without letting abandoned carts starve the store?",
      whyItExists: [
    ...
    understand: {
      overview: "An e-commerce checkout engine uses a 2-stage commit model for inventory: soft reservations during the checkout window, converted to hard decrements only when payment confirmation arrives.",
      components: [
        { name: "Inventory Ledger", whatIsIt: "The authoritative stock record", whyItExists: "Tracks available and reserved counts", whatItDoes: "Answers stock checks" },
        { name: "Reservation Timer", whatIsIt: "Hold expiration tracker", whyItExists: "Frees held stock if user abandons", whatItDoes: "Releases unpurchased items" },
      ],
      analogy: {
        title: "Movie Ticket Hold",
        everyday: ["You click a seat on the screen.", "The app holds it for 5 minutes.", "If you don't pay in time, someone else can book it."],
        technical: ["Click is soft reserve.", "Timer is TTL lock.", "Payment completes transition to BOOKED."],
      },
      flow: ["Check available stock", "Create reservation with expiry timestamp", "Await payment authorization", "Finalize purchase or release hold on timeout"],
    },
    concepts: [
      {
        id: "soft-hold",
        ...
        technicalExplanation: "Stores an item with an expiration timestamp; available stock equals total minus active unexpired reservations.",
    ...
    implementation: {
      ...
      ladder: [
        ...
        { level: "LEVEL 1", title: "Reservation Queue", detail: "Separates available vs reserved." },
        { level: "LEVEL 2", title: "TTL Hold Expiry", detail: "Frees expired holds automatically." },
      ],
      samples: [
        {
          language: "python",
...
class CartCheckout:
    ...
    def clean_expired(self, now: int):
        ...
        for cid in expired:
          ...
          explanations: [
            { code: "if self.stock - active_holds <= 0: return False", explanation: "Checks available stock taking unexpired reservations into account." },
            { code: "self.reservations[cart_id] = now + self.hold_seconds", explanation: "Sets expiration window for the reservation." },
            { code: "self.stock -= 1", explanation: "Converts soft reservation into permanent deduction upon verified checkout." },
          ],
        },
        {
          language: "java",
...
public class CartCheckout {
    ...
    public int getStock() { return stock; }
          ...
          explanations: [
            { code: "reservations.entrySet().removeIf(entry -> now >= entry.getValue());", explanation: "Purges timed-out reservations before stock calculations." },
            { code: "stock--;", explanation: "Permanently deducts inventory after checkout succeeds." },
          ],
        },
        {
          language: "c",
...
bool complete_checkout(CartCheckout *cc, const char *cart_id, long now) {
    ...
    return false;
          ...
          explanations: [
            { code: "if (cc->stock - active_holds <= 0) return false;", explanation: "Validates available inventory against current active reservations." },
            { code: "cc->stock--;", explanation: "Commits inventory deduction upon payment confirmation." },
          ],
        },
      ],
      simulationNote: "Simulates in-memory reservation holds. Real platforms combine Redis TTL keys with database row versioning.",
    },
    practice: [
      { level: "Understand", title: "Trace Reservation", brief: "Trace stock when 2 carts reserve an item with initial stock of 1." },
      { level: "Modify", title: "Extend Hold Window", brief: "Add an API to extend a user's reservation window by 3 minutes." },
      { level: "Build", title: "Multi-Item Cart", brief: "Support reserving lists of SKU items with atomic all-or-nothing hold." },
      { level: "Think", title: "Flash Sale Thundering Herd", brief: "How would you handle 10,000 reservation requests arriving in the exact same second?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "Redis Key Expiration", kind: "Pattern", note: "Modern architectures use Redis SET NX EX to atomically create temporary item locks with automated expiration." },
    ],
    codeLab: {
      title: "Cart Reservation Validator",
      brief: "Write a function process_reservation(stock, active_holds, cart_id, is_checkout, now, expiry). If checking out: return [True, stock - 1, 'CHECKOUT_SUCCESS'] if now < expiry, else [False, stock, 'HOLD_EXPIRED']. If reserving: return [True, active_holds + 1, 'RESERVED'] if stock > active_holds, else [False, active_holds, 'OUT_OF_STOCK'].",
      functionName: "process_reservation",
      signature: "def process_reservation(stock: int, active_holds: int, cart_id: str, is_checkout: bool, now: int, expiry: int) -> tuple:",
    ...
    pass
`,
      javaSignature: "public static Object[] processReservation(int stock, int activeHolds, String cartId, boolean isCheckout, int now, int expiry)",
...
`,
      cSignature: "bool process_reservation(int stock, int active_holds, const char *cart_id, bool is_checkout, int now, int expiry, int *out_val, char *msg)",
...
`,
      ...
      tests: [
        { name: "Successful hold", args: [5, 2, "c1", false, 100, 0], expected: [true, 3, "RESERVED"] },
        { name: "Out of stock hold", args: [2, 2, "c2", false, 100, 0], expected: [false, 2, "OUT_OF_STOCK"] },
        { name: "Valid checkout", args: [5, 2, "c1", true, 150, 200], expected: [true, 4, "CHECKOUT_SUCCESS"] },
        { name: "Expired checkout", args: [5, 2, "c1", true, 250, 200], expected: [false, 5, "HOLD_EXPIRED"] },
      ],
      explanationPrompt: "Explain why checking `now < expiry` on checkout is critical to prevent fulfilling orders whose inventory may have already been released to someone else.",
      mermaid: `graph TD
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    prerequisites: ["Client-Server model", "HTTP headers and status codes", "Event loops / callbacks"],
    engineeringConcepts: ["Full-Duplex Communication", "WebSocket Handshake", "Connection Registry", "Fan-out Broadcasting"],
    technologies: ["WebSockets", "Networking", "Python", "Java", "C"],
    tech: ["ws://", "Handshake", "Room Registry"],
    tags: ["chat", "realtime", "websocket", "fanout"],
    glossary: [
      { term: "Full-Duplex", plainDefinition: "Both sides can send and receive messages simultaneously over the same wire." },
      { term: "Connection Registry", plainDefinition: "In-memory mapping of active connected client sockets by room or user ID." },
    ],
    primers: [
      {
        concept: "Why HTTP Polling Burns Servers",
        minutes: 4,
        definition: "Polling asks 'Got data?' every second. 99% of responses are empty 304s, wasting CPU and battery.",
        whyNeeded: "WebSockets open once and stay idle with zero bandwidth overhead until an actual message arrives.",
        analogy: "Calling your friend every 10 seconds to ask if they called you, vs picking up an open telephone line.",
    ...
    discover: {
      ...
      humanFlow: ["Alice types 'Hello'", "Alice clicks send", "Bob's screen displays 'Hello' in 20ms"],
      question: "How can a server push data to a client when HTTP is designed as a client-pull protocol?",
      whyItExists: ["Low-latency collaboration", "Elimination of wasteful polling headers", "Bidirectional streaming"],
    },
    understand: {
      overview: "WebSockets start as an HTTP request with an 'Upgrade: websocket' header. Once the server accepts, the socket switches to a persistent framing protocol where either party can push bytes anytime.",
      components: [
        { name: "Socket Connection", whatIsIt: "Open TCP tunnel", whyItExists: "Enables instant bidirectional byte stream", whatItDoes: "Carries text frames" },
        { name: "Room Broadcaster", whatIsIt: "Registry of active sockets", whyItExists: "Knows who should receive a message", whatItDoes: "Fans out message to members" },
      ],
      analogy: {
        title: "Walkie-Talkie Channel",
        everyday: ["Everyone tunes to channel 4.", "When you hold the button and speak, everyone listening on channel 4 hears it instantly."],
        technical: ["Channel 4 is the room ID.", "Walkie-talkie speaker is the open WebSocket handle.", "Talking is fan-out broadcast."],
      },
      flow: ["Client initiates HTTP upgrade", "Server verifies and responds with 101 Switching Protocols", "Client joins room", "Publisher sends message", "Server fans out to all sockets in room"],
    },
    concepts: [
      {
        ...
        realWorldAnalogy: "An attendance roll-sheet of who is currently sitting in the conference room.",
        technicalExplanation: "Maps `room_id -> Set[Socket]`. When a client disconnects, its socket is pruned.",
    ...
    implementation: {
      behaviour: "Maintains rooms and broadcasts messages to all joined participants except the sender.",
      algorithm: [
      ...
      ladder: [
        { level: "LEVEL 0", title: "Single Client Echo", detail: "Server reflects message back to same socket." },
        { level: "LEVEL 1", title: "Global Broadcast", detail: "Sends message to all connected clients." },
        { level: "LEVEL 2", title: "Room-Based Fanout", detail: "Scopes delivery to matching room channel." },
      ],
      samples: [
        {
          language: "python",
    ...
    def broadcast(self, room_id: str, sender_id: str, message: str) -> list[str]:
        ...
        return recipients`,
          explanations: [
            { code: "self.rooms[room_id].add(client_id)", explanation: "Registers client socket ID into the room channel." },
            { code: "recipients = [cid for cid in self.rooms[room_id] if cid != sender_id]", explanation: "Filters out the sender so they do not receive an echo of their own message." },
          ],
        },
        {
          language: "java",
...
public class ChatRoomManager {
    ...
    public synchronized List<String> broadcast(String roomId, String senderId) {
        ...
        return recipients;
          ...
          explanations: [
            { code: "rooms.computeIfAbsent(roomId, k -> new HashSet<>()).add(clientId);", explanation: "Creates room bucket lazily and tracks client ID." },
            { code: "if (!cid.equals(senderId)) recipients.add(cid);", explanation: "Calculates recipient targets for fan-out dispatch." },
          ],
        },
        {
          language: "c",
...
int broadcast(const ChatRoom *r, const char *sender_id, char recipients[][32]) {
    ...
    return count;
          ...
          explanations: [
            { code: "if (strcmp(r->members[i], sender_id) != 0)", explanation: "Collects all peers in the room excluding the author." },
          ],
        },
      ],
      simulationNote: "Models room subscription and fanout targets. Production chat servers wire this to socket file descriptors via Redis Pub/Sub.",
    },
    practice: [
      { level: "Understand", title: "Trace Handshake", brief: "List the 3 key HTTP headers required to upgrade to a WebSocket." },
      { level: "Modify", title: "Add Direct Messages", brief: "Modify broadcaster to support sending to a single recipient." },
      { level: "Build", title: "Typing Indicator", brief: "Implement a lightweight ephemeral event for user typing notifications." },
      { level: "Think", title: "Horizontal Scale", brief: "If Alice and Bob are connected to two different web servers, how do they talk?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "Redis Pub/Sub", kind: "Backplane", note: "When chat servers scale horizontally, Redis pub/sub bridges messages between disparate server instances." },
    ],
    codeLab: {
      title: "Chat Fanout Engine",
      brief: "Write a function get_broadcast_recipients(members, sender_id). Return array of member IDs who should receive the message (all members except sender_id). If sender is not in members or members is empty, return empty array.",
    ...
    pass
`,
      javaSignature: "public static String[] getBroadcastRecipients(String[] members, String senderId)",
...
`,
      cSignature: "int get_broadcast_recipients(const char *members[], int count, const char *sender_id, char out[][32])",
...
`,
      ...
      tests: [
        { name: "3 members broadcast", args: [["alice", "bob", "charlie"], "alice"], expected: ["bob", "charlie"] },
        { name: "Sender not in room", args: [["alice", "bob"], "dave"], expected: [] },
        { name: "Single member room", args: [["alice"], "alice"], expected: [] },
      ],
      explanationPrompt: "Explain why the message sender is excluded from the fanout recipient list in chat architectures.",
      mermaid: `graph LR
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    engineeringConcepts: ["Device Tokens", "Gateway Delivery", "Token Invalidation", "Payload Batching"],
    technologies: ["APNs", "FCM", "Python", "Java", "C"],
    tech: ["Device Tokens", "Gateway Dispatch", "Invalidation"],
    tags: ["notifications", "push", "mobile", "fcm"],
    glossary: [
      { term: "Device Token", plainDefinition: "A cryptographic address assigned by iOS/Android representing a specific app installation on a device." },
      { term: "Gateway Provider", plainDefinition: "Operating system cloud infrastructure (APNs/FCM) holding the persistent radio connection to phones." },
    ],
    primers: [
      {
        concept: "Why Apps Can't Stay Open In The Background",
        minutes: 4,
        definition: "Mobile operating systems kill idle background apps to save battery. The OS maintains only ONE shared connection for all apps.",
        whyNeeded: "Instead of 50 apps holding 50 open connections, Apple and Google run one shared tunnel for push packets.",
        analogy: "One postal delivery person visiting an apartment building rather than 50 separate couriers for each tenant.",
        tinyExample: "fcm_gateway.send(token, payload)",
      },
    ],
    discover: {
      situation: "Users need timely alerts (messages, ride arrivals, alerts) even when their phones are locked.",
      humanFlow: ["Driver arrives", "Backend dispatches push", "Lock screen displays notification banner in 1 second"],
      question: "How do servers reach a phone that has closed the app and turned its screen off?",
      whyItExists: ["Zero battery drain for background apps", "Unified OS delivery pipeline", "Reliable delivery guarantees"],
    },
    understand: {
      overview: "When an app installs, the mobile OS grants it a device token. The backend registers this token. To send an alert, the backend sends the payload to APNs/FCM, which delivers it over the phone's single persistent system socket.",
      components: [
        { name: "Token Registry", whatIsIt: "Database of user -> device token mappings", whyItExists: "Knows which phone belongs to whom", whatItDoes: "Stores active tokens" },
        { name: "Gateway Dispatcher", whatIsIt: "HTTP/2 client communicating with Apple/Google", whyItExists: "Transfers alerts to mobile OS clouds", whatItDoes: "Sends batches" },
      ],
      analogy: {
        title: "The Building Mailroom",
        everyday: ["You register your apartment number.", "Packages go to the front desk.", "Front desk rings your buzzer."],
        technical: ["Apartment number is Device Token.", "Front desk is FCM / APNs.", "Buzzer is the lock-screen banner."],
      },
      flow: ["Device sends token to backend", "Backend stores token", "Event occurs", "Backend dispatches payload to APNs/FCM", "Phone receives packet and wakes banner"],
    },
    concepts: [
      {
        ...
        technicalExplanation: "Gateways return HTTP 410 Gone or Unregistered errors; backends must delete invalid tokens.",
        caseApplication: "Prunes inactive tokens from the user's notification list.",
        commonMistakes: ["Repeatedly hammering gateway APIs with dead tokens, resulting in rate limits."],
    ...
    implementation: {
      behaviour: "Dispatches push alerts to registered tokens and handles uninstalled token feedback.",
      algorithm: [
      ...
      ladder: [
        { level: "LEVEL 0", title: "Single Token Delivery", detail: "Direct dispatch to one device." },
        { level: "LEVEL 1", title: "Multi-Device Registry", detail: "Fans out to user's phone + tablet." },
        { level: "LEVEL 2", title: "Invalidation Pruning", detail: "Cleans up uninstalled device tokens." },
      ],
      samples: [
        {
          language: "python",
    ...
    def dispatch(self, user_id: str, invalid_tokens: set = None) -> list[str]:
        ...
        return list(self.user_tokens[user_id])`,
          explanations: [
            { code: "self.user_tokens[user_id].add(token)", explanation: "Associates physical device installation token with user account." },
            { code: "self.user_tokens[user_id] -= invalid_tokens", explanation: "Prunes dead or uninstalled device tokens based on gateway feedback." },
          ],
        },
        {
          language: "java",
...
public class PushNotificationRouter {
    ...
    public synchronized List<String> dispatch(String userId, Set<String> invalidTokens) {
        ...
        return new ArrayList<>(tokens);
          ...
          explanations: [
            { code: "tokens.removeAll(invalidTokens);", explanation: "Cleans up expired device tokens returned by APNs/FCM." },
          ],
        },
        {
          language: "c",
...
int filter_active_tokens(UserDevices *u, const char *invalid_token, char valid_out[][64]) {
    ...
    return valid_count;
          ...
          explanations: [
            { code: "if (strcmp(u->tokens[i], invalid_token) != 0)", explanation: "Filters out inactive device tokens before batch transmission." },
          ],
        },
      ],
    ...
    practice: [
      { level: "Understand", title: "Trace APNs Token", brief: "Why does an app receive a new token when restored from an iCloud backup?" },
      { level: "Modify", title: "Badge Counter", brief: "Add badge number calculation to the notification payload." },
      { level: "Build", title: "Batching Pipeline", brief: "Group 500 tokens into chunks of 100 for gateway API compliance." },
      { level: "Think", title: "Silent Push vs Alert", brief: "What is the difference between a silent data push and an alert banner?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "HTTP/2 Multiplexing", kind: "Transport", note: "APNs uses HTTP/2 multiplexed streams so a single backend TCP socket can transmit thousands of push frames per second." },
    ],
    codeLab: {
      title: "Push Token Dispatcher",
      brief: "Write a function get_valid_push_tokens(tokens, invalid_token). Return list of tokens excluding the invalid_token. If tokens is empty or all are invalid, return empty list.",
    ...
    pass
`,
      javaSignature: "public static String[] getValidPushTokens(String[] tokens, String invalidToken)",
      javaStarterCode: `import java.util.ArrayList;
import java.util.List;
...
`,
      cSignature: "int get_valid_push_tokens(const char *tokens[], int count, const char *invalid_token, char out[][64])",
...
`,
      hints: ["Filter out any token matching invalid_token.", "Return empty array if no valid tokens remain."],
      tests: [
        { name: "Filter one invalid", args: [["tok_a", "tok_b", "tok_c"], "tok_b"], expected: ["tok_a", "tok_c"] },
        { name: "No invalid match", args: [["tok_a", "tok_b"], "tok_z"], expected: ["tok_a", "tok_b"] },
        { name: "All invalid", args: [["tok_bad"], "tok_bad"], expected: [] },
      ],
      explanationPrompt: "Explain how gateway feedback informs the backend when a user uninstalls an app to avoid ghost notification traffic.",
      mermaid: `graph LR
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    glossary: [
      { term: "Sorted Set", plainDefinition: "A collection of unique members ordered by a floating-point score, providing O(log N) insertion and lookup." },
      { term: "Skip List", plainDefinition: "A probabilistic linked list with multiple tiers of pointers enabling binary-search speed on linked elements." },
    ],
    primers: [
      {
        concept: "Why Relational ORDER BY Crumbles Under Gaming Scale",
        minutes: 4,
        definition: "A database table with 10M rows must re-sort or rebuild an index on every score update, creating catastrophic disk lock contention.",
        whyNeeded: "In-memory skip lists update player positions in O(log N) (~23 CPU operations for 10M players).",
        analogy: "An express elevator stopping only at every 10th floor to skip directly to your target tier.",
        tinyExample: "ZADD leaderboard 4500 player_789",
      },
    ],
    discover: {
      situation: "Players finish battle royale matches and expect to see their global rank change instantly.",
      humanFlow: ["Match ends", "Score reported: +150 pts", "Player views global rank: #4,129 (up 312 positions)"],
      question: "How can a system compute the exact rank of any player among 10 million contenders in under 2 milliseconds?",
      whyItExists: ["Instant player dopamine feedback", "Competitive ranking tiers", "Cheater-resistant score ingestion"],
    },
    understand: {
      overview: "Leaderboard engines maintain players inside an in-memory sorted set. Adding or updating a score takes O(log N) time. Querying the Top 10 or finding an arbitrary player's rank takes O(log N) time.",
      components: [
        { name: "Score Index", whatIsIt: "Skip-list sorted data structure", whyItExists: "Keeps elements sorted as they are written", whatItDoes: "Answers rank and range queries" },
        { name: "Player Table", whatIsIt: "Hash map of player_id -> score", whyItExists: "Enables O(1) player score lookups", whatItDoes: "Holds current score values" },
      ],
      analogy: {
        title: "Marathon Finish Line Board",
        everyday: ["Runners cross the line.", "Their chip updates the digital board.", "The board shows both the Top 10 runners and where your friend finished."],
        technical: ["Chip signal is score write.", "Board is the sorted set.", "Friend's rank is ZREVRANK lookup."],
      },
      flow: ["Receive score update", "Update score in hash table", "Re-position player in sorted set", "Return updated rank to player"],
    },
    concepts: [
      {
        id: "sorted-sets",
        ...
        difficulty: "Intermediate",
        simpleDefinition: "A data structure combining a hash table (O(1) lookup) with a skip list (O(log N) ordered traversal).",
        whyItExists: "Provides instant top-K reads and individual rank lookups.",
        realWorldAnalogy: "An alphabetical phone book that also has page bookmarks for every score tier.",
        technicalExplanation: "Maintains unique member keys with ordered score values. Supports range extraction in O(log N + M).",
        caseApplication: "Used to store player gaming scores and compute global standings.",
        commonMistakes: ["Querying an unindexed SQL database with ORDER BY for live gaming leaderboards."],
        practice: ["Calculate the number of operations required to insert into a 1-million node skip list."],
      },
    ],
    architecture: {
    ...
    decisions: [
      {
        ...
        alternatives: ["Periodic batch cron jobs (not real time)", "Redis ZSET (industry standard)"],
    ...
    implementation: {
      ...
      ladder: [
        ...
        { level: "LEVEL 1", title: "Hash + Linear Rank", detail: "O(1) lookup with linear rank count." },
        { level: "LEVEL 2", title: "Top-K Range Extraction", detail: "Fast bounded extraction of leading players." },
      ],
      samples: [
        {
          language: "python",
    ...
    def get_top_k(self, k: int) -> list[tuple[str, int]]:
        ...
        return sorted_players[:k]`,
          explanations: [
            { code: "higher = sum(1 for s in self.scores.values() if s > my_score)", explanation: "Computes 1-based rank by counting players with strictly greater scores." },
            { code: "return sorted_players[:k]", explanation: "Extracts top-K winners for the leaderboard display." },
          ],
        },
        {
          language: "java",
...
public class LiveLeaderboard {
    ...
    public synchronized List<Map.Entry<String, Integer>> getTopK(int k) {
        ...
        return list.subList(0, Math.min(k, list.size()));
          ...
          explanations: [
            { code: "for (int s : scores.values()) if (s > myScore) higher++;", explanation: "Calculates player rank position." },
          ],
        },
        {
          language: "c",
...
int get_player_rank(const Leaderboard *lb, const char *id) {
    ...
    return higher + 1;
          ...
          explanations: [
            { code: "if (lb->players[i].score > target_score) higher++;", explanation: "Computes rank as (number of higher scores + 1)." },
          ],
        },
      ],
      simulationNote: "Models live rank queries. Real systems use Redis ZREVRANGE and ZREVRANK powered by skip-lists.",
    },
    practice: [
      { level: "Understand", title: "Time Complexity", brief: "Why is a skip-list search O(log N) while a linked list is O(N)?" },
      { level: "Modify", title: "Tie Breaking", brief: "Add earlier timestamp tie-breaking when two players have identical scores." },
      { level: "Build", title: "Surrounding Rank Window", brief: "Return the 5 players immediately above and below the user." },
      { level: "Think", title: "Seasonal Reset", brief: "How would you archive and reset the leaderboard every Monday without downtime?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "Redis ZSET", kind: "Standard", note: "Used by virtually all top multiplayer games including Fortnite, PUBG, and Chess.com for instant ratings." },
    ],
    codeLab: {
      title: "Rank Calculator",
      brief: "Write a function compute_player_rank(scores, target_player). Scores is a dict of {player: score}. Return the 1-based rank of target_player (1 is highest). If player not found, return -1.",
    ...
    pass
`,
      javaSignature: "public static int computePlayerRank(String[] players, int[] scores, String targetPlayer)",
      javaStarterCode: `public class Solution {
...
`,
      cSignature: "int compute_player_rank(const char *players[], const int scores[], int count, const char *target_player)",
...
`,
      ...
      tests: [
        { name: "Top score is rank 1", args: [{"alice": 100, "bob": 80, "charlie": 50}, "alice"], expected: 1 },
        { name: "Middle score is rank 2", args: [{"alice": 100, "bob": 80, "charlie": 50}, "bob"], expected: 2 },
        { name: "Lowest score is rank 3", args: [{"alice": 100, "bob": 80, "charlie": 50}, "charlie"], expected: 3 },
        { name: "Player not found", args: [{"alice": 100}, "dave"], expected: -1 },
      ],
      explanationPrompt: "Explain how 1-based rank is derived by counting strictly superior scores.",
      mermaid: `graph TD
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    engineeringConcepts: ["Webhook Delivery", "HMAC Signature Verification", "Exponential Backoff", "Idempotent Retry Queue"],
    technologies: ["Webhooks", "HMAC", "Python", "Java", "C"],
    tech: ["HMAC-SHA256", "Backoff", "Retry Queue"],
    tags: ["webhooks", "retries", "hmac", "reliability"],
    glossary: [
      { term: "Webhook", plainDefinition: "An HTTP callback where a service pushes event notifications to a destination URL provided by the subscriber." },
      { term: "Exponential Backoff", plainDefinition: "A retry strategy that doubles the wait time after each failure (e.g. 1s, 2s, 4s, 8s) to avoid crashing a recovering server." },
    ],
    primers: [
      {
        concept: "The Danger of Linear Retries",
        minutes: 4,
        definition: "Retrying immediately when a server is down acts as a denial-of-service attack that prevents the server from ever recovering.",
        whyNeeded: "Exponential backoff with jitter relieves server congestion and gives the target breathing room to heal.",
        analogy: "If a door is jammed, knocking twice as softly and waiting longer between attempts rather than kicking it down.",
        tinyExample: "wait_seconds = 2 ** attempt_count",
      },
    ],
    discover: {
      situation: "Payment gateways and GitHub push events notify third-party servers via HTTP POST calls.",
      humanFlow: ["Customer pays", "Stripe signs payload with HMAC", "Stripe sends POST to customer URL", "Customer URL responds 200 OK"],
      question: "How does the webhook sender ensure the payload isn't tampered with and arrives even if the destination is temporarily down?",
      whyItExists: ["Asynchronous event integration", "Cross-company microservice coordination", "Tamper-proof payload verification"],
    },
    understand: {
      overview: "A reliable webhook pipeline computes an HMAC-SHA256 signature header so the receiver can verify authenticity. If the receiver fails to return HTTP 2xx, the event is re-queued with exponentially increasing delay.",
      components: [
        { name: "HMAC Signer", whatIsIt: "Cryptographic hash builder", whyItExists: "Proves message integrity", whatItDoes: "Computes signature" },
        { name: "Retry Scheduler", whatIsIt: "Queue with delay timestamps", whyItExists: "Handles receiver downtime", whatItDoes: "Schedules retries" },
      ],
      analogy: {
        title: "Certified Mail with Re-Delivery",
        everyday: ["The mail carrier requires a signature.", "If you are not home, they leave a notice and return tomorrow, then 3 days later."],
        technical: ["Signature is HMAC verification.", "Re-delivery attempts is exponential backoff retry queue."],
      },
      flow: ["Event triggers payload", "Compute HMAC signature", "HTTP POST to target URL", "If status != 200, compute next retry delay", "Re-attempt up to max retries"],
    },
    concepts: [
      {
        ...
        technicalExplanation: "Calculates `delay = base * (2 ^ attempt)`. Caps at a maximum duration.",
    ...
    decisions: [
      {
        ...
        problemSolved: "Avoids knocking out a recovering service with repeated simultaneous retries",
        withoutIt: "Thousands of pending failed retries would DDoS the receiver the moment it turns back on",
    ...
    implementation: {
      behaviour: "Calculates HMAC signature match and schedules retry intervals using exponential backoff.",
      algorithm: [
      ...
      ladder: [
        ...
        { level: "LEVEL 2", title: "Exponential Backoff", detail: "Doubles wait time per attempt up to max limit." },
      ],
      samples: [
        {
          language: "python",
    ...
    def verify_signature(self, payload: str, secret: str, signature: str) -> bool:
        ...
        return expected == signature`,
          explanations: [
            { code: "return self.base_delay * (2 ** attempt)", explanation: "Computes exponential delay growing by powers of 2." },
            { code: "if attempt >= self.max_attempts: return -1", explanation: "Terminates retry loop after maximum attempt budget is exhausted." },
          ],
        },
        {
          language: "java",
    ...
    public boolean verifySignature(String payload, String secret, String signature) {
        ...
        return expected.equals(signature);
          ...
          explanations: [
            { code: "return (int) (baseDelay * Math.pow(2, attempt));", explanation: "Calculates exponential backoff delay in seconds." },
          ],
        },
        {
          language: "c",
...
bool verify_signature(const char *payload, const char *secret, const char *sig) {
    ...
    return strcmp(expected, sig) == 0;
          ...
          explanations: [
            { code: "for (int i = 0; i < attempt; i++) multiplier *= 2;", explanation: "Calculates power-of-two multiplier for backoff." },
          ],
        },
      ],
    ...
    practice: [
      { level: "Understand", title: "Calculate Delays", brief: "List delays for attempts 0 through 4 with base delay of 3 seconds." },
      { level: "Modify", title: "Add Jitter", brief: "Add random jitter (+/- 20%) to avoid synchronized retry waves." },
      { level: "Build", title: "Dead Letter Queue", brief: "Route payloads that failed all attempts into an inspection queue." },
      { level: "Think", title: "At-Least-Once Delivery", brief: "Why must webhook receivers be idempotent?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "Dead Letter Queue (DLQ)", kind: "Architecture", note: "Failed webhooks that exceed maximum retries are moved to a DLQ so developers can inspect and replay them manually." },
    ],
    codeLab: {
      title: "Exponential Backoff Calculator",
      brief: "Write a function get_retry_delay(base_delay, max_attempts, attempt). If attempt >= max_attempts, return -1 (give up). Otherwise return base_delay * (2 ** attempt).",
...
`,
      ...
      explanationPrompt: "Explain how doubling delay time after each attempt gives failing downstream servers breathing room to recover.",
      mermaid: `graph TD
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    engineeringConcepts: ["Token Bucket", "Sliding Window", "Traffic Shaping", "Rate Limit Headers"],
    ...
    glossary: [
      { term: "Token Bucket", plainDefinition: "An algorithm where tokens refill at a steady rate; each request consumes one token, allowing bursts up to bucket capacity." },
      { term: "HTTP 429", plainDefinition: "Standard status code indicating the user has sent too many requests in a given amount of time." },
    ],
    primers: [
      {
        concept: "Fixed Window Reset Spikes",
        minutes: 4,
        definition: "A fixed 1-minute window counter resets at :00. A user sending 100 requests at :59 and 100 at :01 achieves double the quota in 2 seconds.",
        whyNeeded: "Sliding window algorithms prevent reset-boundary traffic surges.",
        analogy: "A toll booth allowing 5 cars per hour: if 5 arrive at 1:59 and 5 arrive at 2:01, 10 cars squeeze through in 2 minutes unless a sliding window is used.",
        tinyExample: "allow = (current_tokens >= 1)",
      },
    ],
    discover: {
      situation: "Public APIs face scraping, credential stuffing, and unintentional infinite loops from client code.",
      humanFlow: ["Client sends requests 1..100 -> HTTP 200", "Client sends request 101 -> HTTP 429 Too Many Requests (Retry-After: 30)"],
      question: "How can API gateways measure and enforce per-client request limits across thousands of concurrent servers?",
      whyItExists: ["Denial-of-Service mitigation", "Monetization tiers (Free vs Pro)", "Fair resource allocation"],
    },
    understand: {
      overview: "A Token Bucket rate limiter tracks token count and last refill timestamp. When a request arrives, elapsed time adds newly generated tokens. If tokens >= 1, the request proceeds; otherwise, HTTP 429 is returned.",
      components: [
        { name: "Bucket State", whatIsIt: "(tokens, last_refill_time)", whyItExists: "Tracks available quota without storing every request timestamp", whatItDoes: "Answers allow/deny" },
        { name: "Refill Engine", whatIsIt: "Time-based replenishment logic", whyItExists: "Restores quota smoothly over time", whatItDoes: "Adds tokens" },
      ],
      analogy: {
        title: "Arcade Token Dispenser",
        everyday: ["The machine drops 1 token into your bowl every 2 seconds.", "Your bowl holds at most 10 tokens.", "Each arcade play costs 1 token. If the bowl is empty, you must wait."],
        technical: ["Bowl is capacity.", "Drop rate is refill rate.", "Playing is consuming a token."],
      },
      flow: ["Request arrives with API key", "Calculate tokens added since last request", "Clamp tokens to maximum capacity", "If tokens >= 1, decrement and return True; else return False"],
    },
    concepts: [
      {
        id: "token-bucket",
        ...
        difficulty: "Advanced",
        simpleDefinition: "An algorithm that allows bursty traffic up to bucket capacity while enforcing a strict long-term rate.",
        whyItExists: "Accommodates real-world bursty browsing while capping maximum sustained throughput.",
        realWorldAnalogy: "A water tank with a steady drip inflow and a tap for quick cup refills.",
        technicalExplanation: "Computes `new_tokens = elapsed * rate`. `current = min(capacity, current + new_tokens)`. If `current >= 1`, decrement and allow.",
        caseApplication: "Used to protect sensitive login and checkout API endpoints.",
        commonMistakes: ["Using background threads to refill buckets instead of lazy on-read math."],
        practice: ["Calculate tokens available after 5 seconds with capacity 10 and rate 2 tokens/sec."],
      },
    ],
    architecture: {
    ...
    implementation: {
      ...
      ladder: [
        ...
        { level: "LEVEL 1", title: "Lazy Token Bucket", detail: "Mathematical refill on arrival without background jobs." },
        { level: "LEVEL 2", title: "Distributed Redis Script", detail: "Atomic Lua script evaluation in Redis." },
      ],
      samples: [
        {
          language: "python",
    ...
    def allow_request(self, client_id: str, now: float) -> bool:
        ...
        if refilled >= 1.0:
            ...
            return False`,
          explanations: [
            { code: "refilled = min(self.capacity, tokens + (elapsed * self.rate))", explanation: "Refills tokens lazily based on elapsed seconds without a background worker." },
            { code: "if refilled >= 1.0: self.clients[client_id] = (refilled - 1.0, now); return True", explanation: "Admits request and consumes 1 token from the available bucket." },
          ],
        },
        {
          language: "java",
...
public class TokenBucketRateLimiter {
    ...
    public synchronized boolean allowRequest(String clientId, double now) {
        ...
        if (refilled >= 1.0) {
          ...
          explanations: [
            { code: "double refilled = Math.min(capacity, st.tokens + (elapsed * ratePerSec));", explanation: "Computes available quota using lazy elapsed-time calculation." },
          ],
        },
        {
          language: "c",
...
bool allow_request(RateLimiter *rl, const char *client_id, double now) {
    ...
    return false;
          ...
          explanations: [
            { code: "if (refilled >= 1.0) { rl->buckets[i].tokens = refilled - 1.0; return true; }", explanation: "Consumes 1 token and permits API access." },
          ],
        },
      ],
    ...
    practice: [
      { level: "Understand", title: "Burst vs Sustained", brief: "With capacity 10 and rate 2/sec, how many requests can arrive in the first second? Over 10 seconds?" },
      { level: "Modify", title: "Cost per Endpoint", brief: "Support expensive API endpoints consuming 5 tokens per call instead of 1." },
      { level: "Build", title: "Headers Generator", brief: "Return X-RateLimit-Remaining and Retry-After headers." },
      { level: "Think", title: "Distributed Race Condition", brief: "If 10 web servers read and write the token count in Redis simultaneously, what race condition occurs?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "Redis Lua Scripts", kind: "Atomicity", note: "Distributed rate limiters evaluate the token bucket inside a single atomic Redis Lua script to eliminate read-modify-write race conditions." },
    ],
    codeLab: {
      title: "Token Bucket Evaluator",
      brief: "Write a function evaluate_token_bucket(capacity, rate, current_tokens, elapsed). Add (elapsed * rate) to current_tokens (capped at capacity). If updated >= 1, return [True, updated - 1, 'ALLOWED']. Otherwise return [False, updated, 'RATE_LIMITED']. Round tokens to 1 decimal place.",
      functionName: "evaluate_token_bucket",
      signature: "def evaluate_token_bucket(capacity: float, rate: float, current_tokens: float, elapsed: float) -> list:",
    ...
    pass
`,
      javaSignature: "public static Object[] evaluateTokenBucket(double capacity, double rate, double currentTokens, double elapsed)",
      javaStarterCode: `public class Solution {
...
`,
      cSignature: "bool evaluate_token_bucket(double capacity, double rate, double current_tokens, double elapsed, double *out_tokens, char *msg)",
...
`,
      ...
      tests: [
        { name: "Full capacity allows", args: [5.0, 1.0, 5.0, 0.0], expected: [true, 4.0, "ALLOWED"] },
        { name: "Empty bucket rate limited", args: [5.0, 1.0, 0.2, 0.0], expected: [false, 0.2, "RATE_LIMITED"] },
        { name: "Refill permits request", args: [5.0, 1.0, 0.2, 1.0], expected: [true, 0.2, "ALLOWED"] },
      ],
      explanationPrompt: "Explain how lazy token recalculation avoids needing a background timer thread for every active user.",
      mermaid: `graph TD
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    glossary: [
      { term: "Visibility Timeout", plainDefinition: "A timer where a job leased by a worker becomes invisible to others; if the worker crashes before completion, the job reappears." },
    ],
    primers: [
      {
        concept: "The 30-Second Web Request Timeout",
        minutes: 4,
        definition: "Web servers and proxies (Cloudflare/Nginx) abort HTTP requests that take more than 15-30 seconds to answer.",
        whyNeeded: "Heavy jobs (video encoding, report generation) must immediately return an ID and run in background workers.",
        analogy: "Getting a claim ticket at the dry cleaner and coming back later instead of standing at the counter for 3 hours while they wash your coat.",
        tinyExample: "return { 'status': 'QUEUED', 'job_id': 'job_123' }",
      },
    ],
    discover: {
      situation: "Users submit long operations (video transcode, batch emails) that cannot finish in 200ms.",
      humanFlow: ["Upload video", "Server responds in 50ms with 'Processing'", "Background worker encodes video", "Notification signals completion"],
      question: "How do web services return immediate HTTP responses while reliable worker machines process heavy computations in the background?",
      whyItExists: ["Zero HTTP timeouts", "Independent worker autoscaling", "Crash-safe retry queues"],
    },
    understand: {
      overview: "Web servers enqueue job specifications into a broker. Independent worker processes pull jobs, process them, and update a status store. The frontend polls `GET /jobs/:id` until complete.",
      components: [
        { name: "Message Broker", whatIsIt: "FIFO queue", whyItExists: "Buffers jobs safely", whatItDoes: "Stores task messages" },
        { name: "Worker Process", whatIsIt: "Background computation loop", whyItExists: "Executes heavy workload", whatItDoes: "Pulls and executes tasks" },
      ],
      analogy: {
        title: "Restaurant Kitchen Order Wheel",
        everyday: ["Waiter clips order ticket to the wheel.", "Waiter returns to customer immediately.", "Chefs take tickets in order and cook."],
        technical: ["Waiter is Web Server.", "Ticket wheel is Message Queue.", "Chefs are Background Workers."],
      },
      flow: ["Web server enqueues job", "Returns HTTP 202 Accepted with job_id", "Worker pulls job from queue", "Worker executes and saves result", "Client polls or receives webhook"],
    },
    concepts: [
      {
        ...
        difficulty: "Advanced",
        simpleDefinition: "A period during which a task leased by a worker is hidden from other workers.",
        whyItExists: "If a worker crashes mid-job, the job automatically reappears on the queue so another worker can complete it.",
        realWorldAnalogy: "Giving someone a library book for 2 weeks; if they never return it, the library flags it as lost and reorders it.",
        technicalExplanation: "Prevents duplicate concurrent execution while guaranteeing crash recovery without lost tasks.",
        caseApplication: "Used by Amazon SQS and RabbitMQ for safe message delivery.",
        commonMistakes: ["Setting visibility timeout shorter than average job execution time, causing duplicates."],
    ...
    decisions: [
      {
        ...
        withoutIt: "A few heavy uploads would exhaust web server thread pools and freeze the whole site",
    ...
    implementation: {
      ...
      ladder: [
        ...
        { level: "LEVEL 1", title: "Status Tracking", detail: "State transitions: Queued -> Running -> Done." },
    ...
    def get_status(self, job_id: str) -> str:
        return self.jobs.get(job_id, "NOT_FOUND")`,
          explanations: [
            { code: "self.queue.append(job_id)", explanation: "Buffers task ID into FIFO processing queue." },
            { code: "self.jobs[job_id] = 'COMPLETED'", explanation: "Updates task status so client polling gets confirmation." },
          ],
        },
        {
          language: "java",
...
public class BackgroundJobQueue {
    ...
    public synchronized String getStatus(String jobId) {
        return jobs.getOrDefault(jobId, "NOT_FOUND");
          ...
          explanations: [
            { code: "String jobId = queue.remove(0);", explanation: "De-queues oldest task in FIFO order for execution." },
          ],
        },
        {
          language: "c",
...
const char* process_job(JobQueue *q) {
    ...
    return "NO_JOBS";
          ...
          explanations: [
            { code: "strcpy(q->jobs[i].status, \"COMPLETED\");", explanation: "Transitions first queued job to completed state." },
          ],
        },
      ],
    ...
    practice: [
      { level: "Understand", title: "Trace Polling", brief: "Why does the client poll GET /jobs/:id instead of keeping the connection open?" },
      { level: "Modify", title: "Failed State", brief: "Add a FAILED status and error message string when processing encounters an exception." },
      { level: "Build", title: "Priority Queue", brief: "Support HIGH and LOW priority job queues." },
      { level: "Think", title: "Poison Pill", brief: "What happens if a job has a bug that crashes every worker that touches it?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "BullMQ & Celery", kind: "Standard", note: "BullMQ (Node/Redis) and Celery (Python/RabbitMQ) are the primary background task engines in modern web platforms." },
    ],
    codeLab: {
      title: "Job State Transitioner",
      brief: "Write a function transition_job(current_state, event). Events can be 'START' or 'FINISH'. If state is 'QUEUED' and event is 'START', return 'RUNNING'. If state is 'RUNNING' and event is 'FINISH', return 'COMPLETED'. Any other combination is invalid, return 'INVALID_TRANSITION'.",
...
`,
      ...
      tests: [
        ...
        { name: "Cannot finish queued job", args: ["QUEUED", "FINISH"], expected: "INVALID_TRANSITION" },
        { name: "Cannot start running job", args: ["RUNNING", "START"], expected: "INVALID_TRANSITION" },
      ],
      explanationPrompt: "Explain how strict state transitions prevent workers from executing or completing the same job more than once.",
      mermaid: `graph LR
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    engineeringConcepts: ["Circuit Breaker", "Fail-Fast", "Half-Open Probe", "Cascading Failure Mitigation"],
    ...
    glossary: [
      { term: "Circuit Breaker", plainDefinition: "A design pattern that detects failures and encapsulates the logic of preventing a failure from constantly recurring." },
      { term: "Fail Fast", plainDefinition: "Immediately returning an error or cached fallback without wasting time attempting an operation known to be failing." },
    ],
    primers: [
      {
        concept: "The Anatomy of a Cascading Outage",
        minutes: 4,
        definition: "If Service A calls Service B with a 10s timeout, and Service B hangs, Service A's 100 worker threads freeze waiting for B. Soon Service A runs out of threads and crashes too.",
        whyNeeded: "Failing fast in 1ms preserves your server's health while downstream services reboot.",
        analogy: "An electrical circuit breaker in your home tripping to cut current before overheating wires start an electrical fire.",
    ...
    discover: {
      ...
      humanFlow: ["User opens home page", "Recommendation engine is down", "Circuit breaker trips", "Home page loads in 50ms with default products instead of freezing for 30s"],
      question: "How do we prevent one failing dependency from dragging down the entire microservice ecosystem?",
      whyItExists: ["Thread pool protection", "Degraded graceful mode operation", "Automated dependency recovery detection"],
    },
    understand: {
      overview: "A circuit breaker monitors calls. In CLOSED state, calls pass through. When failures exceed a threshold, it trips to OPEN, rejecting all calls immediately. After a cooldown, it enters HALF-OPEN to send test probes.",
      components: [
        { name: "Failure Counter", whatIsIt: "Sliding failure window tracker", whyItExists: "Detects persistent failure rate", whatItDoes: "Trips breaker when threshold met" },
        { name: "Cooldown Timer", whatIsIt: "Recovery countdown", whyItExists: "Gives downstream service time to reboot", whatItDoes: "Transitions to HALF-OPEN" },
      ],
      analogy: {
        title: "Electrical Breaker",
        everyday: ["Too many appliances overload the wire.", "The switch flips off.", "You unplug appliances, wait, and cautiously flip the switch back on."],
        technical: ["Overload is downstream failure.", "Switch flip is OPEN state.", "Cautiously testing is HALF-OPEN probe."],
      },
      flow: ["Calls succeed (CLOSED)", "5 consecutive failures occur", "Breaker trips to OPEN (fail fast)", "Wait 30 seconds cooldown", "Send 1 trial call (HALF-OPEN)", "If trial succeeds -> CLOSED; else -> OPEN"],
    },
    concepts: [
      {
        ...
        difficulty: "Advanced",
        simpleDefinition: "CLOSED (operating normally), OPEN (failing fast immediately), and HALF-OPEN (testing trial request).",
        whyItExists: "Provides graceful degradation and automated self-healing.",
        realWorldAnalogy: "Green light (go), Red light (stop), Yellow blinking light (proceed with caution).",
        technicalExplanation: "State transitions are triggered by failure ratios and cooldown expiration timestamps.",
    ...
    implementation: {
      behaviour: "Tracks consecutive failures and enforces state transitions between CLOSED, OPEN, and HALF-OPEN.",
      algorithm: [
      ...
      ladder: [
        ...
        { level: "LEVEL 1", title: "Binary Open/Close", detail: "Trips open when threshold reached." },
        { level: "LEVEL 2", title: "Half-Open Recovery", detail: "Probes downstream service to automatically heal." },
      ],
      samples: [
        {
          language: "python",
    ...
    def record_result(self, success: bool, now: int):
        if success:
          ...
          explanations: [
            { code: "if now >= self.open_until: self.state = 'HALF_OPEN'; return True", explanation: "Allows a single probe request after cooldown expires." },
            { code: "return False # Fail fast", explanation: "Immediately rejects request without waiting on the failing dependency." },
          ],
        },
        {
          language: "java",
    ...
    public String getState() { return state; }
          ...
          explanations: [
            { code: "if (failures >= threshold || \"HALF_OPEN\".equals(state))", explanation: "Trips to OPEN if threshold exceeded or test probe fails." },
          ],
        },
        {
          language: "c",
...
void record_result(CircuitBreaker *cb, bool success, long now) {
    if (success) {
        ...
        if (cb->failures >= cb->threshold || cb->state == CB_HALF_OPEN) {
          ...
          explanations: [
            { code: "if (cb->state == CB_OPEN && now < cb->open_until) return false;", explanation: "Enforces immediate fail-fast error rejection." },
          ],
        },
      ],
    ...
    practice: [
      { level: "Understand", title: "Trace Transitions", brief: "What state does the breaker enter after 3 consecutive failures? After cooldown?" },
      { level: "Modify", title: "Fallback Value", brief: "Return cached stale data when can_attempt() returns false." },
      { level: "Build", title: "Percentage Threshold", brief: "Trip when failure rate exceeds 50% over a 100-request window." },
      { level: "Think", title: "Cascading Cascade", brief: "What happens if the fallback service itself fails?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "Resilience4j & Envoy", kind: "Standard", note: "Resilience4j in Java and Envoy Service Mesh proxies implement Circuit Breaking at the network layer transparently." },
    ],
    codeLab: {
      title: "Circuit Breaker State Machine",
      brief: "Write a function evaluate_breaker(state, failures, threshold, is_success, now, open_until). Return [new_state, new_failures, new_open_until]. If is_success is True, reset to ['CLOSED', 0, 0]. If is_success is False, increment failures. If failures + 1 >= threshold, trip to ['OPEN', failures + 1, now + 30]. Otherwise return [state, failures + 1, open_until].",
      functionName: "evaluate_breaker",
      signature: "def evaluate_breaker(state: str, failures: int, threshold: int, is_success: bool, now: int, open_until: int) -> list:",
      starterCode: `def evaluate_breaker(state, failures, threshold, is_success, now, open_until):
    # Enforce circuit breaker transitions
    pass
`,
      javaSignature: "public static Object[] evaluateBreaker(String state, int failures, int threshold, boolean isSuccess, int now, int openUntil)",
      javaStarterCode: `public class Solution {
...
`,
      cSignature: "bool evaluate_breaker(const char *state, int failures, int threshold, bool is_success, int now, int open_until, char *out_state, int *out_fail, int *out_until)",
...
`,
      ...
      tests: [
        { name: "Success resets to closed", args: ["OPEN", 3, 3, true, 100, 130], expected: ["CLOSED", 0, 0] },
        { name: "Failure trips open", args: ["CLOSED", 2, 3, false, 100, 0], expected: ["OPEN", 3, 130] },
        { name: "Failure below threshold", args: ["CLOSED", 0, 3, false, 100, 0], expected: ["CLOSED", 1, 0] },
      ],
      explanationPrompt: "Explain how resetting failures to 0 on success allows the circuit breaker to recover cleanly.",
      mermaid: `graph TD
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    engineeringConcepts: ["Liveness Probes", "Readiness Probes", "Flap Dampening", "Dynamic Pool Failover"],
    ...
    glossary: [
      { term: "Liveness Probe", plainDefinition: "A check that tests whether an application process has locked up and needs to be restarted." },
      { term: "Readiness Probe", plainDefinition: "A check that tests whether an application is fully loaded (database connected, cache warm) and ready to receive customer traffic." },
    ],
    primers: [
      {
        concept: "The Flapping Server Problem",
        minutes: 4,
        definition: "If a single packet drop marks a server dead, and the next packet marks it alive, the load balancer 'flaps' routing tables continuously.",
        whyNeeded: "Requiring 3 consecutive failures to evict and 2 consecutive successes to admit stabilizes routing.",
        analogy: "A doctor taking your pulse three times to be sure, rather than declaring an emergency because of one missed beat.",
    ...
    discover: {
      ...
      humanFlow: ["Server 3 crashes", "Health checker detects 3 missed probes in 6 seconds", "Load balancer removes Server 3 from routing table", "0% of users experience broken pages"],
      question: "How do load balancers detect and remove dead servers before customers notice broken requests?",
      whyItExists: ["99.99% uptime guarantees", "Zero-downtime rolling deployments", "Automated cluster self-healing"],
    },
    understand: {
      overview: "Health monitors ping `/healthz` on each backend server every 2 seconds. Counters track consecutive results. When a server fails 3 consecutive checks, it is removed from the active pool. When it passes 2 in a row, it is restored.",
      components: [
        { name: "Health Prober", whatIsIt: "Synthetic heartbeat pinger", whyItExists: "Tests instance responsiveness", whatItDoes: "Sends HTTP GET /healthz" },
        { name: "Routing Pool", whatIsIt: "List of vetted live instances", whyItExists: "Directs customer requests safely", whatItDoes: "Distributes web traffic" },
      ],
      analogy: {
        title: "Airport Runway Inspector",
        everyday: ["An inspector drives down the runway checking for debris.", "If debris is spotted, flights are diverted to runway 2 immediately."],
        technical: ["Inspector is Health Prober.", "Diverting flights is Load Balancer Pool Failover."],
      },
      flow: ["Prober pings server", "Server returns HTTP 200 -> reset failure count", "Server returns 500 or times out -> increment failure count", "If failures >= 3, evict from active pool"],
    },
    concepts: [
      {
        id: "flap-dampening",
        ...
        whyItExists: "Prevents transient network blips from constantly churn-rebuilding load balancer tables.",
        realWorldAnalogy: "Requiring three strikes before an umpire calls an out in baseball.",
        technicalExplanation: "Enforces hysteresis: `unhealthy_threshold = 3`, `healthy_threshold = 2`.",
        caseApplication: "Used in Kubernetes, AWS ELB, and HAProxy backend configurations.",
        commonMistakes: ["Failing over immediately on 1 failed ping during a temporary network switch blip."],
    ...
    decisions: [
      {
        ...
        problemSolved: "Passive sniffing requires users to experience broken requests before errors are detected",
    ...
    implementation: {
      ...
      ladder: [
        ...
        { level: "LEVEL 1", title: "Consecutive Thresholds", detail: "Dampens flapping with fail/pass counts." },
        { level: "LEVEL 2", title: "Readiness vs Liveness", detail: "Separates traffic drain from container restart." },
      ],
      samples: [
        {
          language: "python",
    ...
    def get_active_pool(self) -> list[str]:
        return [sid for sid, s in self.servers.items() if s["status"] == "HEALTHY"]`,
          explanations: [
            { code: "if s['fails'] >= self.fail_threshold: s['status'] = 'UNHEALTHY'", explanation: "Evicts server from active routing pool after 3 consecutive failures." },
            { code: "if s['passes'] >= self.pass_threshold: s['status'] = 'HEALTHY'", explanation: "Restores server to active pool only after 2 consecutive clean health checks." },
          ],
        },
        {
          language: "java",
...
public class ServerHealthTracker {
    ...
    public synchronized List<String> getActivePool() {
        ...
        return pool;
          ...
          explanations: [
            { code: "if (n.fails >= 3) n.status = \"UNHEALTHY\";", explanation: "Prunes unhealthy node to protect customer traffic." },
          ],
        },
        {
          language: "c",
...
void record_probe(HealthTracker *ht, const char *id, bool is_up) {
    for (int i = 0; i < ht->count; i++) {
        if (strcmp(ht->nodes[i].id, id) == 0) {
          ...
          explanations: [
            { code: "if (ht->nodes[i].fails >= 3) ht->nodes[i].is_healthy = false;", explanation: "Updates health status with hysteresis threshold." },
          ],
        },
      ],
    ...
    practice: [
      { level: "Understand", title: "Flapping Prevention", brief: "Why is requiring 2 consecutive passes better than immediate re-admission on 1 pass?" },
      { level: "Modify", title: "Dynamic Weights", brief: "Reduce server traffic weight by 50% on first failure before complete eviction." },
      { level: "Build", title: "Deep Health Check", brief: "Verify database read/write capability inside /healthz rather than just process ping." },
      { level: "Think", title: "Cascading Eviction", brief: "If 4 out of 5 servers fail, should the load balancer evict all 4 and crash the 5th?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "Kubernetes Probes", kind: "Standard", note: "Kubelet queries livenessProbe (restart on failure) and readinessProbe (route traffic only on success) independently." },
    ],
    codeLab: {
      title: "Failover Status Evaluator",
      brief: "Write a function update_node_health(status, fails, passes, is_probe_ok). If is_probe_ok: passes = passes + 1, fails = 0, status = 'HEALTHY' if passes >= 2 else status. If not is_probe_ok: fails = fails + 1, passes = 0, status = 'UNHEALTHY' if fails >= 3 else status. Return [status, fails, passes].",
      functionName: "update_node_health",
      signature: "def update_node_health(status: str, fails: int, passes: int, is_probe_ok: bool) -> list:",
      starterCode: `def update_node_health(status, fails, passes, is_probe_ok):
    # Enforce hysteresis failover rules
    pass
`,
      javaSignature: "public static Object[] updateNodeHealth(String status, int fails, int passes, boolean isProbeOk)",
      javaStarterCode: `public class Solution {
...
`,
      cSignature: "bool update_node_health(const char *status, int fails, int passes, bool is_probe_ok, char *out_status, int *out_fails, int *out_passes)",
...
`,
      ...
      tests: [
        { name: "3rd failure evicts node", args: ["HEALTHY", 2, 0, false], expected: ["UNHEALTHY", 3, 0] },
        { name: "1st failure keeps healthy", args: ["HEALTHY", 0, 0, false], expected: ["HEALTHY", 1, 0] },
        { name: "2nd pass restores healthy", args: ["UNHEALTHY", 0, 1, true], expected: ["HEALTHY", 0, 2] },
        { name: "1st pass remains unhealthy", args: ["UNHEALTHY", 0, 0, true], expected: ["UNHEALTHY", 0, 1] },
      ],
      explanationPrompt: "Explain why resetting the opposite counter to 0 on every probe result prevents lingering stale counts from corrupting threshold decisions.",
      mermaid: `graph TD
  ...
  // ──────────────────────────────────────────────────────────────────────────
  {
    ...
    engineeringConcepts: ["Idempotency Keys", "Duplicate Request Deduplication", "Race Condition Locking", "Replay Response Caching"],
    ...
    glossary: [
      { term: "Idempotency", plainDefinition: "A property of an API request where making the exact same call multiple times produces the exact same result as making it once." },
      { term: "Idempotency Key", plainDefinition: "A unique client-generated UUID sent in headers (Idempotency-Key: xxx) identifying a single intended transaction." },
    ],
    primers: [
      {
        concept: "The Two-Generals Network Dilemma in Billing",
        minutes: 4,
        definition: "If you send $100 to the bank and the internet drops, you do not know if the bank received your money or if the connection dropped before arrival.",
        whyNeeded: "Without idempotency keys, resending the request risks billing the user twice.",
        analogy: "Writing a check number on a payment; the bank cashes check #1042 once, and rejects any second attempt to cash #1042.",
        tinyExample: "headers = {'Idempotency-Key': 'order_789_uuid'}",
      },
    ],
    discover: {
      situation: "Shoppers on cellular connections experience intermittent signal drops while clicking 'Confirm Order'.",
      humanFlow: ["User clicks 'Pay $100'", "Server charges card", "WiFi disconnects before response reaches phone", "User clicks 'Pay $100' again in panic", "User is charged only once"],
      question: "How can payment APIs guarantee that duplicate submissions from flaky networks never double-charge customer credit cards?",
      whyItExists: ["Financial compliance", "Consumer trust and chargeback prevention", "Network resilience across mobile networks"],
    },
    understand: {
      overview: "When a payment request arrives with `Idempotency-Key`, the server checks if that key has been seen. If already processed, it returns the cached receipt immediately. If currently processing, it locks. If new, it executes and stores the response.",
      components: [
        { name: "Idempotency Store", whatIsIt: "Key-value ledger of key -> response", whyItExists: "Caches confirmed payment receipts", whatItDoes: "Answers replays" },
        { name: "Transaction Lock", whatIsIt: "Mutual exclusion lock per key", whyItExists: "Prevents concurrent double-clicks from running in parallel", whatItDoes: "Serializes duplicate requests" },
      ],
      analogy: {
        title: "Unique Invoice Number",
        everyday: ["Your landlord gives you invoice #901.", "You pay it.", "If your roommate brings another check for invoice #901, the landlord says 'Already paid!'"],
        technical: ["Invoice #901 is the Idempotency Key.", "Landlord checking the ledger is response replay."],
      },
      flow: ["Client generates unique UUID", "Sends POST /charge with Idempotency-Key", "Server checks ledger: if seen, return cached receipt", "If unseen, acquire lock, charge bank, save receipt, return response"],
    },
    concepts: [
      {
        id: "idempotency-keys",
        ...
        difficulty: "Advanced",
        simpleDefinition: "A unique token sent by the client that allows the server to recognize retried requests.",
        whyItExists: "Makes non-idempotent HTTP POST operations safely retryable.",
        realWorldAnalogy: "A passport number: having 2 copies of your passport does not make you 2 different citizens.",
        technicalExplanation: "The server stores `(idempotency_key, payload_hash, response_status, response_body)`. Identical keys replay the stored response.",
        ...
        practice: ["Design what response to return if the same key is reused with a different dollar amount."],
      },
    ],
    architecture: {
    ...
    decisions: [
      {
        ...
        withoutIt: "Network timeouts before the response would require creating fresh unlinked transactions",
    ...
    implementation: {
      behaviour: "Stores executed payments by idempotency key and replays receipts on duplicate submissions.",
      algorithm: [
      ...
      ladder: [
        ...
        { level: "LEVEL 2", title: "Concurrent Locks", detail: "Locks key to reject parallel double-clicks." },
      ],
      samples: [
        {
          language: "python",
    ...
    def process_charge(self, idempotency_key: str, amount: int) -> dict:
        ...
        return {
          ...
          explanations: [
            { code: "if idempotency_key in self.ledger: return { ... 'is_replay': True }", explanation: "Returns the original receipt without executing a second credit card charge." },
            { code: "self.ledger[idempotency_key] = ...", explanation: "Records the transaction result keyed by the client's idempotency key." },
          ],
        },
        {
          language: "java",
...
public class IdempotentPaymentGateway {
    ...
    public synchronized Map<String, Object> processCharge(String idempotencyKey, int amount) {
        ...
        return res;
          ...
          explanations: [
            { code: "if (ledger.containsKey(idempotencyKey))", explanation: "Deduplicates retried payments using synchronized ledger lookup." },
          ],
        },
        {
          language: "c",
...
bool process_charge(PaymentGateway *pg, const char *key, int amount, char *out_tx_id, bool *is_replay) {
    ...
    *is_replay = false;
    ...
    return true;
          ...
          explanations: [
            { code: "if (strcmp(pg->records[i].key, key) == 0) { *is_replay = true; return true; }", explanation: "Replays existing transaction ID on identical idempotency key." },
          ],
        },
      ],
    ...
    practice: [
      { level: "Understand", title: "Two-Generals Problem", brief: "Why is it mathematically impossible to guarantee both parties know the message arrived over an unreliable network?" },
      { level: "Modify", title: "Payload Mismatch Check", brief: "Return 422 Unprocessable Entity if the key is reused with a different amount." },
      { level: "Build", title: "Redis Distributed Lock", brief: "Implement a 30-second SET NX lock during the charge phase." },
      { level: "Think", title: "Idempotency TTL", brief: "How long should idempotency keys be remembered in the database? 24 hours? 7 days? Forever?" },
    ],
    reflection: [
    ...
    techNotes: [
      { name: "Stripe Idempotency Spec", kind: "Standard", note: "Stripe's API popularized the Idempotency-Key HTTP header, requiring 24-hour persistence and payload match validation." },
    ],
    codeLab: {
      title: "Idempotent Charge Engine",
      brief: "Write a function execute_idempotent_charge(seen_keys, key, amount). If key is in seen_keys, return [True, seen_keys[key], 'REPLAY']. Otherwise generate tx_id = 'tx_' + key, save to seen_keys, and return [True, tx_id, 'CHARGED'].",
    ...
    pass
`,
      javaSignature: "public static Object[] executeIdempotentCharge(java.util.Map<String, String> seenKeys, String key, int amount)",
      javaStarterCode: `public class Solution {
...
`,
      cSignature: "bool execute_idempotent_charge(const char *seen_keys[][2], int seen_count, const char *key, int amount, char *out_tx, char *out_status)",
...
`,
      ...
      tests: [
        { name: "First charge executes", args: [{}, "k1", 100], expected: [true, "tx_k1", "CHARGED"] },
        { name: "Duplicate key returns replay", args: [{"k1": "tx_k1"}, "k1", 100], expected: [true, "tx_k1", "REPLAY"] },
        { name: "Second unique key executes", args: [{"k1": "tx_k1"}, "k2", 50], expected: [true, "tx_k2", "CHARGED"] },
      ],
      explanationPrompt: "Explain how returning the cached transaction receipt prevents the credit card from being debited twice.",
      mermaid: `graph TD
