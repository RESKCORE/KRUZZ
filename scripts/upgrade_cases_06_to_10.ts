...
const CASE_ENRICHMENTS: Record<string, any> = {
  "seat-booking-system": {
    discover: {
      situation:
        "Two fans try to reserve row F seat 12 at the exact same millisecond for a sold-out blockbuster movie.",
      humanFlow: [
      ...
      question:
        "How does a booking engine prevent double-booking without making every customer wait in a slow global line?",
      whyItExists: [
    ...
    understand: {
      overview:
        "A seat booking system maintains an authoritative state machine for every seat in a venue. When a user taps a seat, the engine attempts an atomic state transition from AVAILABLE to RESERVED with an expiration timer. Once payment succeeds, the seat transitions to BOOKED; if payment fails or the timer expires, the hold is safely returned to AVAILABLE.",
      components: [
        {
      ...
      analogy: {
        ...
        story:
          "When you hand over your jacket, the attendant places a numbered wooden peg in the slot immediately so no other attendee gets that peg while your receipt is written.",
        everyday: [
          "Select coat peg",
          "Attendant holds peg",
          "Pay fee",
          "Receive numbered claim slip",
        ],
        technical: [
          "Request seat ID",
          "Set seat to RESERVED state with TTL",
          "Validate payment transaction",
          "Set seat to BOOKED and return ticket",
        ],
        mapping: [
          ...
          {
            everyday: "Receive numbered claim slip",
            technical: "Generate immutable ticket barcode",
          },
        ],
      },
      flow: [
    ...
    implementation: {
      ...
      ladder: [
        {
          step: 1,
          focus: "Binary Seat Map",
          buildsOn: "In-memory boolean array tracking booked vs available",
        },
        {
          step: 2,
          focus: "State Machine with Timers",
          buildsOn: "Three-state model (AVAILABLE, RESERVED, BOOKED) with timestamps",
        },
        {
          step: 3,
          focus: "Atomic Concurrency Guard",
          buildsOn: "Mutex locking and compare-and-swap to defeat race conditions",
        },
      ],
    },
    practice: [
      {
        level: "Understand",
        title: "Trace Concurrent Collisions",
        brief: "Diagram what happens when two threads call book_seat() simultaneously for seat 4.",
      },
      {
        level: "Modify",
        title: "Add VIP Row Logic",
        brief:
          "Restructure the reservation check to require a loyalty tier flag for rows A through C.",
      },
      {
        level: "Build",
        title: "Implement Timeout Sweeper",
        brief:
          "Write a cleanup routine that automatically releases reservations older than 10 minutes.",
      },
      {
        level: "Think",
        title: "Distributed Seat Locking",
        brief:
          "Evaluate how Redis distributed locks (Redlock) or SQL SELECT FOR UPDATE behave during a ticket frenzy.",
      },
    ],
    reflection: [
    ...
    techNotes: [
      {
        ...
        kind: "Concurrency",
        note: "Pessimistic locking locks the database row during the checkout flow; optimistic locking uses version numbers at final commit time.",
      },
    ],
  },
  ...
  "inventory-stock-tracker": {
    discover: {
      situation:
        "A flash sale launches 500 units of a limited keyboard. 10,000 checkout requests hit the server within 3 seconds.",
      humanFlow: [
      ...
      question:
        "How do high-volume commerce engines guarantee that available stock never goes below zero during peak traffic spikes?",
      whyItExists: [
    ...
    understand: {
      ...
      analogy: {
        ...
        story:
          "A mechanical candy dispenser only turns its gear if a physical coin is inserted; if the chamber is empty, the mechanical arm physically stops and refuses to advance.",
        everyday: [
          "Insert token",
          "Gear verifies coin presence",
          "Dispense item",
          "Coin drops into locked box",
        ],
        technical: [
          "Receive decrement request",
          "Check stock >= requested quantity",
          "Subtract quantity atomically",
          "Append entry to audit journal",
        ],
        mapping: [
          { everyday: "Insert token", technical: "Order checkout request" },
          {
            everyday: "Gear verifies coin presence",
            technical: "Stock threshold check (count >= qty)",
          },
    ...
    implementation: {
      ...
      ladder: [
        {
          step: 1,
          focus: "Direct Counter",
          buildsOn: "In-memory dictionary with basic subtraction and floor checks",
        },
        {
          step: 2,
          focus: "Atomic Guard",
          buildsOn: "Conditional compare-and-swap decrement logic",
        },
        {
          step: 3,
          focus: "Audit Ledger",
          buildsOn: "Append-only transaction journaling for balance reconciliation",
        },
      ],
    },
    practice: [
      {
        level: "Understand",
        title: "Stock Starvation Analysis",
        brief:
          "Calculate the probability of overselling when read and write operations are not transactional.",
      },
      {
        level: "Modify",
        title: "Support Backorders",
        brief: "Allow inventory to decrement past zero only if an allowBackorders flag is enabled.",
      },
      {
        level: "Build",
        title: "Multi-Warehouse Stock Allocator",
        brief:
          "Write an algorithm that fulfills an order from the warehouse geographically closest to the customer.",
      },
      {
        level: "Think",
        title: "Eventual Consistency in Warehousing",
        brief:
          "Analyze how Amazon reconciles physical inventory audits with digital ledger records.",
      },
    ],
    reflection: [
    ...
    techNotes: [
      {
        name: "Atomic Decrements",
        kind: "Data Integrity",
        note: "Using SQL 'UPDATE inventory SET count = count - 1 WHERE id = 1 AND count >= 1' prevents negative balances natively.",
      },
    ],
  },
  ...
  "client-server-architecture": {
    discover: {
      situation:
        "You type https://kruzz.dev into your browser. Within 50 milliseconds, layout, styles, and data render on your screen from a server located 2,000 miles away.",
      humanFlow: [
      ...
      question:
        "How does the web separate user interaction from data storage and business computation across distributed machines?",
      whyItExists: [
    ...
    understand: {
      ...
      components: [
        ...
        },
        {
          ...
          whatItDoes:
            "Parses headers, validates payloads, and serializes response status and bodies.",
        },
      ],
      analogy: {
        title: "Restaurant Dining Experience",
        story:
          "You sit at a dining table (Client) and tell the waiter your order. The waiter brings the ticket to the kitchen (Server). The kitchen prepares the dish using pantry ingredients (Database) and the waiter serves your meal (Response).",
        everyday: [
          "Review menu",
          "Place order with waiter",
          "Kitchen cooks meal",
          "Waiter delivers hot plate",
        ],
        technical: [
          "Render UI",
          "Dispatch HTTP GET /menu",
          "Server queries database and formats JSON",
          "Client renders received payload",
        ],
        mapping: [
    ...
    implementation: {
      ...
      ladder: [
        {
          step: 1,
          focus: "Static Route Matcher",
          buildsOn: "Exact string path matching against handler dictionary",
        },
        {
          step: 2,
          focus: "Status Code Protocol",
          buildsOn: "Supporting 200 OK, 400 Bad Request, 404 Not Found",
        },
        {
          step: 3,
          focus: "Dynamic Parameter Parsing",
          buildsOn: "Extracting query strings and URL slugs for handler execution",
        },
      ],
    },
    practice: [
      {
        level: "Understand",
        title: "Trace an HTTP Handshake",
        brief: "Write out the 3-way TCP SYN/ACK handshake that precedes the first HTTP request.",
      },
      {
        level: "Modify",
        title: "Add Route Parameters",
        brief: "Enhance the router to parse dynamic path variables like /users/:id.",
      },
      {
        level: "Build",
        title: "Build a Health Check Endpoint",
        brief: "Implement a /health route that verifies server uptime and memory consumption.",
      },
      {
        level: "Think",
        title: "Thick Client vs Thin Client",
        brief:
          "Compare Single Page Applications (SPAs) where client does rendering vs Server-Side Rendering (SSR).",
      },
    ],
    reflection: [
    ...
    techNotes: [
      {
        name: "Statelessness of HTTP",
        kind: "Networking",
        note: "Each HTTP request is independent; servers do not keep socket state open between requests unless explicitly upgraded to WebSockets.",
      },
    ],
  },
  ...
  "dns-domain-lookup": {
    discover: {
      situation:
        "Humans remember names like kruzz.dev, but network routers only understand IP addresses like 104.21.45.12.",
      humanFlow: [
      ...
      question:
        "How does the global internet translate billions of human-readable domain names into IP addresses in single-digit milliseconds?",
      whyItExists: [
    ...
    understand: {
      overview:
        "The Domain Name System (DNS) is the phonebook of the internet. It maps human-friendly hostnames (e.g. kruzz.dev) to machine-routable IP addresses. Resolution works via a tree hierarchy: local cache -> recursive resolver -> root servers (.) -> TLD servers (.dev) -> authoritative nameservers.",
      components: [
        {
      ...
      analogy: {
        ...
        story:
          "If you need to call 'Alice in Seattle', you first ask the national directory for Washington State, then Washington directs you to Seattle municipal records, who gives you Alice's exact phone number.",
        everyday: [
          "Check personal contacts",
          "Call regional operator",
          "Operator consults city ledger",
          "Receive dialable telephone number",
        ],
        technical: [
          "Check local browser DNS cache",
          "Query Root DNS for TLD .dev",
          "Query .dev TLD for authoritative server",
          "Query authoritative server for A record IP",
        ],
        mapping: [
    ...
    implementation: {
      ...
      ladder: [
        {
          step: 1,
          focus: "In-Memory Host Table",
          buildsOn: "Static hostname-to-IP dictionary resolution",
        },
        {
          step: 2,
          focus: "TTL Cache Expiration",
          buildsOn: "Validating cache timestamps against current time",
        },
        {
          step: 3,
          focus: "Recursive Traversal Simulator",
          buildsOn: "Emulating Root -> TLD -> Authoritative delegation hops",
        },
      ],
    },
    practice: [
      {
        level: "Understand",
        title: "Trace Dig Query",
        brief:
          "Analyze the output of `dig +trace kruzz.dev` through root, TLD, and authoritative hops.",
      },
      {
        level: "Modify",
        title: "Implement CNAME Resolution",
        brief:
          "Extend the DNS lookup to recursively follow canonical name aliases until an A record is resolved.",
      },
      {
        level: "Build",
        title: "Build an LRU DNS Cache",
        brief:
          "Implement an in-memory DNS cache that evicts the least-recently-used entry when capacity is reached.",
      },
      {
        level: "Think",
        title: "DNS Poisoning & DNSSEC",
        brief:
          "Evaluate how cryptographic DNSSEC signatures prevent attackers from spoofing malicious IP addresses.",
      },
    ],
    reflection: [
    ...
    techNotes: [
      {
        name: "UDP vs TCP in DNS",
        kind: "Protocols",
        note: "Standard DNS queries use UDP port 53 for lightning-fast lightweight lookups; responses exceeding 512 bytes fallback to TCP.",
      },
    ],
  },
  ...
  "image-cdn-delivery": {
    discover: {
      situation:
        "A 4MB high-res image hosted on a server in Virginia takes 2.4 seconds to load for a user in Tokyo due to 14,000 miles of cross-Pacific fiber latency.",
      humanFlow: [
      ...
      question:
        "How do modern media platforms deliver rich photos and video assets to worldwide users in under 50 milliseconds?",
      whyItExists: [
    ...
    understand: {
      overview:
        "A Content Delivery Network (CDN) is a globally distributed network of proxy servers (Points of Presence) deployed in data centers worldwide. When a user requests an image, Anycast DNS routes them to the geographically closest edge server. If the edge has the image cached, it returns it in single-digit milliseconds; if not, it fetches it once from origin and caches it for all subsequent users.",
      components: [
        {
      ...
      analogy: {
        ...
        story:
          "Instead of every citizen driving 200 miles to a central farm to buy a gallon of milk, local corner grocery stores stock milk daily so you can walk 3 minutes to pick it up.",
        everyday: [
          "Walk to neighborhood grocery",
          "Check shelf for milk",
          "If in stock, buy immediately",
          "If empty, store manager orders from central farm",
        ],
        technical: [
          "Route request to nearest Edge PoP",
          "Check Edge SSD cache for image hash",
          "On hit, stream directly to user",
          "On miss, fetch from Origin S3 bucket and cache",
        ],
        mapping: [
    ...
    implementation: {
      ...
      ladder: [
        {
          step: 1,
          focus: "Edge Hit/Miss Proxy",
          buildsOn: "In-memory cache checking before delegating to origin",
        },
        {
          step: 2,
          focus: "Cache-Control Expiration",
          buildsOn: "Enforcing max-age and stale-while-revalidate headers",
        },
        {
          step: 3,
          focus: "Origin Shielding",
          buildsOn: "Adding intermediate parent cache layers to collapse origin traffic",
        },
      ],
    },
    practice: [
      {
        level: "Understand",
        title: "Calculate Latency Savings",
        brief:
          "Compare round-trip time (RTT) for a 50ms edge connection versus a 240ms cross-Pacific origin connection.",
      },
      {
        level: "Modify",
        title: "Add Cache-Control Headers",
        brief: "Configure response headers for immutable images (max-age=31536000, immutable).",
      },
      {
        level: "Build",
        title: "Simulate Cache Invalidation",
        brief:
          "Write a purge endpoint that invalidates edge entries matching a regex path pattern.",
      },
      {
        level: "Think",
        title: "Thundering Herd Problem",
        brief:
          "Analyze how edge nodes protect the origin when 50,000 simultaneous users request a newly released image on a cold cache.",
      },
    ],
    reflection: [
    ...
    techNotes: [
      {
        name: "Anycast BGP Routing",
        kind: "Networking",
        note: "Anycast assigns the same IP address to hundreds of edge nodes worldwide; internet routers automatically route packets to the nearest node.",
      },
    ],
  },
};
